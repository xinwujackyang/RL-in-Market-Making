...
def train_seed(
    ...
    if cfg.total_steps != 100_352:
        raise ValueError("PPO mechanism screening must run exactly 100,352 steps")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Two-condition PPO mechanism screening")
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "results" / "ppo_mechanism_screen",
    )
    parser.add_argument("--resume", action="store_true")
    args = parser.parse_args()

    if args.seeds != 3:
        raise ValueError("PPO mechanism screening requires exactly three seeds")
    ...
    for condition, n_epochs, ent_coef in CONDITIONS:
