...
TARGET_CHECKPOINTS = (10_000, 20_000, 40_000, 60_000, 100_000, 150_000, 200_000)
...
def rollout_checkpoints(horizon: int, total_steps: int) -> set[int]:
    checkpoints = {
...
def plot_trajectories(
    ...
    seeds = sorted({int(row["seed"]) for row in trajectory})
    for axis, key, title in zip(axes, keys, titles):
        for seed in seeds:
            selected = sorted(
                [row for row in trajectory if int(row["seed"]) == seed],
        ...
        axis.axhline(
            benchmark,
            color="black",
            linestyle="--" if benchmark == 0.0 else ":",
            linewidth=1.0,
            label=benchmark_label,
        )
        axis.set(title=title, xlabel="Training step")
        axis.legend(fontsize=8, ncol=2)
    axes[0].set_ylabel("Mean epsilon" if benchmark == 0.0 else "Latent sigma")
    figure.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Five-seed long-run Random convergence study")
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "results" / "long_run_random")
    ...
    for seed in range(args.seeds):
        if seed in completed:
            ...
            continue
        print(f"Training long-run seed={seed}", flush=True)
    ...
    plot_trajectories(
        trajectory,
        args.output_dir / "mean_policy_long_run.png",
        ("deterministic_epsilon_bid_mean", "deterministic_epsilon_ask_mean"),
        ("Deterministic bid mean", "Deterministic ask mean"),
        0.0,
        "epsilon*=0",
    )
    plot_trajectories(
        trajectory,
        args.output_dir / "policy_std_long_run.png",
        ("latent_sigma_bid", "latent_sigma_ask"),
        ("Latent sigma bid", "Latent sigma ask"),
        1.0,
        "initial sigma=1",
    )
