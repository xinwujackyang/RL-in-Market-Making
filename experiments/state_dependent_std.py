...
def main() -> None:
    parser = argparse.ArgumentParser(description="Custom PPO with state-dependent policy std")
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "results" / "state_dependent_std",
    )
    ...
    for seed in range(args.seeds):
        if seed in completed:
            ...
            continue
        print(f"Training state-dependent-std seed={seed}", flush=True)
        ...
        print(
            ...
            f"{row['deterministic_epsilon_ask_mean']:.3f}), latent sigma mean="
