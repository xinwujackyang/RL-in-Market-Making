...
def train_seed(
    ...
    if cfg.total_steps != 100_352:
        raise ValueError("Beta screening must run exactly 100,352 training steps")
    if max(SCREENING_CHECKPOINTS) != cfg.total_steps:
        raise ValueError("final screening checkpoint must equal total training steps")
    ...
    history = agent.train(
        ...
        evaluation_steps=SCREENING_CHECKPOINTS,
    )
        ...
        trajectory_rows.append(
            {
                ...
                "deterministic_error_bid": abs(evaluation["mean_epsilon_bid"]),
                "deterministic_error_ask": abs(evaluation["mean_epsilon_ask"]),
                "two_side_mae": (
                    abs(evaluation["mean_epsilon_bid"])
                    + abs(evaluation["mean_epsilon_ask"])
                )
                / 2.0,
    ...
    metrics = {
        ...
        "deterministic_error_bid": abs(deterministic_bid),
        "deterministic_error_ask": abs(deterministic_ask),
        ...
        "epsilon_near_bound_frequency": sampled_metrics["epsilon_near_bound_frequency"],
        "hedge_near_bound_frequency": sampled_metrics["hedge_near_bound_frequency"],
        ...
        "policy_distribution": cfg.policy_distribution,
        "initial_beta_concentration": 1.54,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Three-seed Beta bounded-policy screening")
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "results" / "beta_policy_screen",
    )
    ...
    for seed in range(args.seeds):
        if seed in completed:
            print(f"Skipping completed Beta seed={seed}", flush=True)
            continue
        print(f"Training Beta policy seed={seed}", flush=True)
        ...
        print(
            ...
            f"{row['two_side_mae']:.3f}, near-bound="
            f"{row['epsilon_near_bound_frequency']:.3f}",
            flush=True,
        )
