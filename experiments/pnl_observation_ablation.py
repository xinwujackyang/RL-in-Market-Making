...
def train_seed(
    ...
    metrics = {
        ...
        "include_fill_feedback": cfg.include_fill_feedback,
        "paper_pnl_observation": cfg.paper_pnl_observation,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Long-run PPO without TotalPnL and HedgeCost observations"
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "results" / "pnl_observation_ablation",
    )
    ...
    for seed in range(args.seeds):
        if seed in completed:
            ...
            continue
        print(f"Training PnL-observation ablation seed={seed}", flush=True)
