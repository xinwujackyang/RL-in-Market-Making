...
def main() -> None:
    parser = argparse.ArgumentParser(description="Custom PPO with GAE lambda-return targets")
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "results" / "gae_value_target",
    )
    ...
    for seed in range(args.seeds):
        if seed in completed:
            ...
            continue
        print(f"Training GAE-value-target seed={seed}", flush=True)
